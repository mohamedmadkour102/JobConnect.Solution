...
using JobDto = JobConnect.Application.DTOs.EmployerDto.JobDto;
...
namespace JobConnect.Application.Services
{
    public class EmployerService : IEmployerService
    {
        ...
        {
            var employer = await _uow.Employers.GetEmployerByIdAsync(employerId);
            if (employer == null)
        ...
        public async Task UpdateCompanyInfoAsync(string employerId, UpdateCompanyInfoDto dto)
        {
            ...
            _uow.Employers.UpdateEmployer(employer);
        ...
        {
            var jobs = await _uow.Employers.GetRecentJobsByEmployerAsync(employerId);
        ...
        {
            var jobs = await _uow.Employers.GetJobsByEmployerAsync(employerId);
        ...
        {
            var job = await _uow.Employers.GetJobByIdAsync(jobId, employerId);
        ...
        public async Task AddJobAsync(string employerId, CreateJobDto jobDto)
        {
            ...
            await _uow.Employers.AddJobAsync(job);
            await _uow.SaveChangesAsync();
        }
        ...
        public async Task UpdateJobAsync(int jobId, string employerId, UpdateJobDto jobDto)
        {
            ...
            _uow.Employers.UpdateJob(job);
            await _uow.SaveChangesAsync();
        }

        public async Task DeleteJobAsync(int jobId, string employerId)
        {
            await _uow.Employers.DeleteJobAsync(jobId, employerId);
            await _uow.SaveChangesAsync();
        }
        ...
        public async Task<JobStatsDto> GetJobStatsAsync(string employerId)
        {
            ...
            {
                JobsCount = await _uow.Employers.GetJobsCountAsync(employerId),
                CandidatesCount = await _uow.Employers.GetCandidatesCountAsync(employerId)
            };
        }

        public async Task AddToShortlistAsync(int jobId, string jobSeekerId)
        {
            await _uow.Employers.AddToShortlistAsync(jobId, jobSeekerId);
            await _uow.SaveChangesAsync();
        }

        public async Task RemoveFromShortlistAsync(int jobId, string jobSeekerId)
        {
            await _uow.Employers.RemoveFromShortlistAsync(jobId, jobSeekerId);
            await _uow.SaveChangesAsync();
        }

        public async Task<IEnumerable<ShortlistedJobSeekerDto>> GetShortlistedJobSeekersAsync(int jobId, string employerId)
        {
            var applications = await _uow.Employers.GetShortlistedJobSeekersAsync(jobId, employerId);
        ...
        {
            var (jobs, totalCount) = await _uow.Employers.GetJobsByEmployerPaginatedAsync(employerId, pageNumber, pageSize);
        ...
        public async Task DeleteEmployerAccountAsync(string employerId)
        {
            ...
            {
                await _uow.Employers.DeleteJobAsync(job.Id, employerId);
            }
        ...
        public async Task<SeekerProfileDto?> GetJobSeekerByIdAsync(string employerId, string jobSeekerId)
        {
            ...
            var jobSeeker = await _uow.Employers.GetJobSeekerByIdAsync(jobSeekerId);
            if (jobSeeker == null)
                return null;
         ...
         public async Task<IEnumerable<JobApplicantWithResumeDto>> GetApplicantsWithResumeAsync(int jobId, string employerId)
        {
            ...
            var applications = await _uow.Employers.GetApplicationsByJobAsync(jobId);
            if (!applications.Any())
        ...
        public async Task<bool> HireApplicantAsync(string employerId, int jobId, string jobSeekerId)
        {
            ...
            var ok = await _uow.Employers.HireApplicantAsync(jobId, jobSeekerId);
            if (!ok)
                return false;
            ...
            await _notificationService.SendNotificationToUserAsync(
                ...
                {
                    Title = "تم قبول طلبك!",
                    Message = $"تم قبول طلبك للوظيفة {job.Title}",
        ...
        public async Task<bool> RejectApplicantAsync(string employerId, int jobId, string jobSeekerId)
        {
            ...
            var ok = await _uow.Employers.RejectApplicantAsync(jobId, jobSeekerId);
            if (!ok)
                return false;
            ...
            await _notificationService.SendNotificationToUserAsync(
                ...
                {
                    Title = "تم رفض طلبك",
                    Message = $"تم رفض طلبك للوظيفة {job.Title}",
                    Type = NotificationType.ApplicationStatus,
                    DataJson = JsonSerializer.Serialize(new { jobId = job.Id }),
                    RedirectUrl = $"/jobs/{job.Id}"
                });
