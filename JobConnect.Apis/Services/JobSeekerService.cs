...
//					LogoBase64 = await FileHelper.ConvertRelativeFileToBase64Async(_environment.WebRootPath, job.Employer.LogoUrl)
//				}
//			}));
//		}
        ...
        public async Task<IEnumerable<JobDto>> GetAllJobsAsync()
        {
            ...
            return await Task.WhenAll(jobs.Select(async job => new JobDto
            {
                ...
                Employer = new EmployerInfo
                {
                    ...
                    FoundingDate = job.Employer.FoundingDate,
                    Industry = job.Employer.Industry,
                    LogoBase64 = string.IsNullOrEmpty(job.Employer.LogoUrl) ? null : job.Employer.LogoUrl // استخدام اللينك بدل Base64
                }
            }));
        }
